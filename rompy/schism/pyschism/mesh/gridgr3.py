...
class Gr3Field(Gr3):
    ...
    def modify_by_region(self, hgrid, fname, value, depth1, flag):
        ...
        points = [Point(*coord) for coord in self.coords]
        gdf2 = gpd.GeoDataFrame({"geometry": points, "index": list(range(len(points)))})
        gdf_in = gpd.sjoin(gdf2, gdf1, op="within")
        picks = [i.index for i in gdf_in.itertuples()]
        if flag == 0:
            self.values[picks] = value
...
class Shapiro(Gr3Field):
    ...
    def slope_filter(
        ...
        if fp:
            ...
            fpn = ~mask
            dpedx2 = (
                v1[fpn] * (y3[fpn] - y4[fpn])
                + v3[fpn] * (y4[fpn] - y1[fpn])
                + v4[fpn] * (y1[fpn] - y3[fpn])
            ) / (2 * a2[fpn])
            dpedy2 = (
                (x4[fpn] - x3[fpn]) * v1[fpn]
                + (x1[fpn] - x4[fpn]) * v3[fpn]
                + (x3[fpn] - x1[fpn]) * v4[fpn]
            ) / (2 * a2[fpn])
        ...
        if len(depths) != len(shapiro_vals1):
            raise Exception(
                f"lengths of depths {len(depths)} and shapiro_vals1 {len(shapiro_vals1)} inconsistent"
            )
        fp = dp < depths[-1]
