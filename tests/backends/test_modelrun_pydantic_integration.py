...
class TestModelRunPydanticIntegration:
    ...
    def test_run_with_local_config_using_config_run_method(self, model_run_with_run_method, tmp_path):
    ...
    def test_docker_config_validation_in_modelrun_context(self, model_run, tmp_path):
        ...
        with pytest.raises(ValueError, match="Either 'image' or 'dockerfile' must be provided"):
            DockerConfig()

        # Test mutual exclusion of image and dockerfile
        with pytest.raises(ValueError, match="Cannot specify both 'image' and 'dockerfile'"):
