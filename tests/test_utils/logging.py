...
def configure_test_logging(
    ...
    if level is None:
        ...
        level = LogLevel[level_name] if level_name in LogLevel.__members__ else LogLevel.INFO
    ...
    if format_str is None:
        ...
        format_str = LogFormat[format_str.upper()] if format_str.upper() in LogFormat.__members__ else LogFormat.VERBOSE
    
    # Configure basic logging
    logging.basicConfig(level=level.value, format="%(asctime)s - %(name)s - %(levelname)s - %(message)s")
